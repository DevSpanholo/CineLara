...
class ContaController extends Controller {
	...
	public function getFormRelatorio() {
		...
		$pessoas      = [];//Fornecedor::where('cliente', '1')->get();
		$fornecedores = [];//Fornecedor::where('fornecedor', '1')->get();
		$users        = [];//User::all();
	...
	public function listarReceber() {
		...
		$contas = $this->contaModel->newQuery()
				->where('tipo_operacao', 'R')
				->where('vlr_restante', '>', '0.00')
				->with('user', 'parcelas', 'parcelasPagas')
				->get();
	...
	public function buscaContas(Request $request) {
		$contas = $this
			->contaModel
			->newQuery()
			->where('tipo_operacao', $request->input('tipo'))
			->where('vlr_restante', '>', '0.00')
			->with('pessoa', 'parcelas', 'parcelasPagas')
			->get();
	...
	public function getFormAdicionarReceber() {
		...
		$pessoas = $this->userModel->newQuery()->get();
	...
	public function getFormAdicionarPagar() {
		...
		$pessoas = $this->fornecedorModel->newQuery()->get();
	...
	public function gravar(ContaRequest $request) {
		try {
			...
			$tipo = $request->get('tipo_operacao');
			$input                 = $request->all();
			if ($tipo == 'R') {
				$input['user_id']      = $input['pessoa_id'];
			} else {
				$input['fornecedor_id']= $input['pessoa_id'];
			}
		
			unset($input['data_emissao']);
			$input['vlr_restante'] = $input['vlr_total'];
			$conta                 = Conta::create($input);
			$this->gravaParcelas($input['array_parcela'], $conta->id);
			DB::commit();
			return redirect()->route('contas.'.($tipo == 'R'?'receber':'pagar').'.listar')
	...
	public function getFormAlterarReceber($id) {
		try {
			...
			$pessoas = $this->fornecedorModel->newQuery()->where('cliente', 1)->get();
